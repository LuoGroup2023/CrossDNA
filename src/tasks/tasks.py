...
class LMTask(BaseTask):
    def forward(self, batch, encoder, model, decoder, _state):
        ...
        # decoder output typically has .logits
        x = x.logits

        def _is_streaming_pack(p):
            # streaming pack: (ce_sum(0d), n_masked(0d), total_aux(0d), stats, step)
            return (
                isinstance(p, (tuple, list))
                and len(p) >= 3
                and torch.is_tensor(p[0]) and p[0].dim() == 0
                and torch.is_tensor(p[1]) and p[1].dim() == 0
            )

        if isinstance(x, tuple):  # x comes out from bert lm head as a tuple pack
            x = list(x)

            #  streaming pack: do NOT rearrange x[0] (it's scalar ce_sum)
            if _is_streaming_pack(x):
                # y still can be flattened (loss may ignore it in streaming mode)
                y = rearrange(y, '... -> (...)')
                return x, y, w

            #  old pack: x[0] is fused logits [..., C]
            x[0] = rearrange(x[0], '... C -> (...) C')

        else:
            # plain logits tensor [..., C]
            x = rearrange(x, '... C -> (...) C')

        # keep old behavior for labels
        y = rearrange(y, '... -> (...)')
        return x, y, w

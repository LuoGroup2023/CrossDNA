...
class ClassificationHead(nn.Module):
    def __init__(self, d_model, d_output, dropout=0.2):
        super().__init__()
...
class SequenceDecoder(Decoder):
    def __init__(self, d_model, d_output=None, l_output=None, use_lengths=False, mode="last"):
        super().__init__()
        #print('d_model:', d_model,"d_output:", d_output)
        self.output_transform = nn.Identity() if d_output is None else nn.Linear(d_model, d_output)
        # head_in_dim = d_model * 2 if mode == "pool" else d_model
        # self.output_transform = nn.Identity() if d_output is None else ClassificationHead(head_in_dim, d_output)
        if l_output is None:
    ...
    def forward(self, x, state=None, lengths=None, l_output=None, mask=None):
        ...
        #     if mask is None:
        #         def restrict(tt):
        #             mean_feat = tt.mean(dim=-2, keepdim=True)          # [B,1,D]
        #             max_feat = tt.max(dim=-2, keepdim=True).values     # [B,1,D]
        #             return torch.cat([mean_feat, max_feat], dim=-1)    # [B,1,2D]
        #     else:
        #         def restrict(tt):
        #             # mask: [B,L], 有效位置为1
        #             m = mask.to(tt.dtype).unsqueeze(-1)                # [B,L,1]
        #             denom = m.sum(dim=-2, keepdim=True).clamp_min(1.0)
        #             mean_feat = (tt * m).sum(dim=-2, keepdim=True) / denom

        #             neg_inf = torch.finfo(tt.dtype).min
        #             tt_masked = tt.masked_fill(~mask.bool().unsqueeze(-1), neg_inf)
        #             max_feat = tt_masked.max(dim=-2, keepdim=True).values
        ...
        #print('x_out shape after restrict:', x_out.shape)

...
def cross_entropy(logits, y, ignore_index=-100, label_smoothing=0.0):
    logits = logits.view(-1, logits.shape[-1])
    y = y.view(-1)
    return F.cross_entropy(logits, y, ignore_index=ignore_index, label_smoothing=label_smoothing)
...
# 分块loss
# def streaming_cross_entropy_with_semantic_and_dual(hyout, labels, report_topk=(1, 3)):
#     """
#     A) streaming pack:
#         (ce_sum, n_masked, total_aux, stats, step)
#         stats can be:
#           - tuple/list of tensors
#           - OR 1D tensor [keep_rate, c1, c3, a1, b1, a3, b3]
...
#     # ----------------------------
#     if (
#         isinstance(pack, (tuple, list))
#         and len(pack) >= 3
#         and torch.is_tensor(pack[0]) and pack[0].dim() == 0
#         and torch.is_tensor(pack[1]) and pack[1].dim() == 0
#     ):
#         ce_sum = pack[0]
#         n_masked = pack[1]
#         total_aux = pack[2]
#         stats = pack[3] if len(pack) > 3 else None

#         denom = max(1, int(n_masked.item()))
#         ce = ce_sum / denom
#         ppl = math.exp(ce.item()) if torch.isfinite(ce) else float("inf")

#         keep_rate = float("nan")
#         c1 = c3 = a1 = b1 = a3 = b3 = 0.0

#         if stats is not None:
#             if torch.is_tensor(stats):
#                 if stats.dim() == 0:
#                     keep_rate = float(stats.item())
#                 else:
#                     if stats.numel() >= 1:
#                         keep_rate = float(stats[0].item())
#                     if stats.numel() >= 2:
#                         c1 = float(stats[1].item())
#                     if stats.numel() >= 3:
#                         c3 = float(stats[2].item())
#                     if stats.numel() >= 4:
#                         a1 = float(stats[3].item())
#                     if stats.numel() >= 5:
#                         b1 = float(stats[4].item())
#                     if stats.numel() >= 6:
#                         a3 = float(stats[5].item())
#                     if stats.numel() >= 7:
#                         b3 = float(stats[6].item())
#             elif isinstance(stats, (tuple, list)) and len(stats) > 0:
#                 def _to_float(x, default=0.0):
#                     try:
#                         if torch.is_tensor(x):
#                             return float(x.item()) if x.dim() == 0 else float(x.reshape(-1)[0].item())
#                         return float(x)
#                     except Exception:
#                         return float(default)

#                 keep_rate = _to_float(stats[0], default=float("nan"))
#                 if len(stats) >= 2:
#                     c1 = _to_float(stats[1])
#                 if len(stats) >= 3:
#                     c3 = _to_float(stats[2])
#                 if len(stats) >= 4:
#                     a1 = _to_float(stats[3])
#                 if len(stats) >= 5:
#                     b1 = _to_float(stats[4])
#                 if len(stats) >= 6:
#                     a3 = _to_float(stats[5])
#                 if len(stats) >= 7:
#                     b3 = _to_float(stats[6])

#         msgs = [f"CE {ce.item():.3f}", f"ppl_masked {ppl:.3f}", f"mask_rate {keep_rate:.2%}"]
#         if 1 in report_topk:
#             msgs.append(f"Acc@1 fused {100.0 * (c1 / denom):.2f}%")
#         if 3 in report_topk:
#             msgs.append(f"Acc@3 fused {100.0 * (c3 / denom):.2f}%")

#         if 1 in report_topk:
#             msgs.append(f"A {100.0 * (a1 / denom):.2f}% B {100.0 * (b1 / denom):.2f}% (top-1)")
#         if 3 in report_topk:
#             msgs.append(f"A {100.0 * (a3 / denom):.2f}% B {100.0 * (b3 / denom):.2f}% (top-3)")
...
def streaming_cross_entropy_with_semantic_and_dual(hyout, labels, report_topk=(1, 3)):
    ...
    if (
        ...
        # 关键修正：
        # streaming 分支里 labels 往往是 full target，不是 mlm_labels
        # 所以 mask_rate 必须用 n_masked / 总 token 数来算
        if torch.is_tensor(labels) and labels.numel() > 0:
            keep_rate = float(n_masked.item()) / float(labels.numel())
        else:
            keep_rate = float("nan")
...
output_metric_fns = {
    ...
    "streaming_cross_entropy_with_semantic_and_dual": streaming_cross_entropy_with_semantic_and_dual,

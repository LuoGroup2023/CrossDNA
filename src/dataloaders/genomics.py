...
class BertMultiSpecies(SequenceDataset):
    """
    Multi-species data loader that inherits from SequenceDataset.
    Supports loading multiple genomes and their corresponding BED files.
    """
    ###### very important to set this! ######
    _name_ = "bert_multi_species" 

    def __init__(self, 
                 species_list: List[str],  # 传入物种列表，例如 ['hg38', 'mm10']
                 tokenizer_name=None, 
        ...
        for species in self.species_list:
            # 假设目录结构为: data/bert_multi_species/{species}/...
            species_dir = default_data_path / self._name_ / species
            self.species_files[species] = {
                'bed': species_dir / f'{species}-sequences.bed',
                'fasta': species_dir / f'{species}.fa'
            }

        # Fault tolerant & DDP logic (保持原样)
        self.fault_tolerant = fault_tolerant
        if self.shuffle and self.fault_tolerant: pass 
        self.ddp = ddp
        self.fast_forward_epochs = fast_forward_epochs
        self.fast_forward_batches = fast_forward_batches
    ...
    def init_datasets(self):
        """Init and concatenate datasets for all species."""
        train_sets, val_sets, test_sets = [], [], []

        for species in self.species_list:
            files = self.species_files[species]
            print(f"Initializing datasets for {species} with files: {files}")
            # 为每个物种创建对应的三个 split
            # 注意：此处 BertHG38Dataset 需要能处理不同物种的 bed/fasta
            curr_splits = [
                BertHG38Dataset(  # 建议重命名为更通用的 DNAIntervalDataset
                    split=split,
                    bed_file=files['bed'],
                ...
                for split, max_len in zip(['train', 'valid', 'test'], 
                                          [self.max_length, self.max_length_val, self.max_length_test])
            ]
            train_sets.append(curr_splits[0])
            val_sets.append(curr_splits[1])
            test_sets.append(curr_splits[2])

        # 使用 ConcatDataset 将多物种数据合为一体
        self.dataset_train = ConcatDataset(train_sets)
        self.dataset_val = ConcatDataset(val_sets)
        self.dataset_test = ConcatDataset(test_sets)

    def train_dataloader(self, **kwargs) -> DataLoader:
        if self.shuffle and self.fault_tolerant:
            sampler = (FaultTolerantDistributedSampler(self.dataset_train, **self.trainer.distributed_sampler_kwargs)
                       if self.ddp else RandomFaultTolerantSampler(self.dataset_train))
            shuffle = False
        else:
            sampler = None

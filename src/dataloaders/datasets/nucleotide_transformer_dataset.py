...
class NucleotideTransformerDataset(torch.utils.data.Dataset):
    ...
    - A/C/G/T/N -> 0/1/2/3/4
    - no special token insertion when use_tokenizer=False
    - optional attention mask for padded positions
    """
    ...
    def __init__(
        ...
        duplicate_action: str = "first",
        token_id_offset: int = 7,
        pad_value_id: int = 4,
        **unused_kwargs,
    ):
        self.max_length = int(max_length)
        self.use_padding = True if use_padding is None else bool(use_padding)
        self.tokenizer_name = tokenizer_name
        self.tokenizer = tokenizer
        self.return_augs = return_augs
        self.add_eos = add_eos
    ...
    def _encode_with_tokenizer(self, x: str):
        ...
        # keep backward compatibility with old char-tokenizer path
        ids = input_ids - self.token_id_offset
        ids[(ids < 0) | (ids > 4)] = 4
        input_ids = ids
    ...
    def __getitem__(self, idx):
        ...
        if self.use_tokenizer:
            input_ids, attn_mask = self._encode_with_tokenizer(x)
        else:
            input_ids, attn_mask = self._encode_direct(x)
